...
async function save() {
  ...
  if (!v$.value.$invalid) {
    ...
    if (resultado.error) {
      ...
      equipe.id_equipe = resultado.master
      toast.success(msg)
    }
  } else {
...
var isEditMode = computed(() => Number(equipe.id_equipe) > 0)
