      ...
      <div class="column is-11">
        ...
        <div class="card" style="min-height: 60vh">
          <header class="card-header">
            <p class="card-header-title is-centered">Imóveis Cadastrados</p>
...
const STORAGE_KEY = 'consulta-imovelsw'
...
async function loadData() {
  try {
    ...
    const result = await imovelService.getImovels(JSON.stringify(filter))
    if (result.error) {
...
async function onEditRow(item) {
  router.push(`/imovel/${item.row.id}`)
}
...
async function onDeleteRow(item) {
  ...
  if (ok) {
    const resultado = await imovelService.delete(item.row.id)
    if (resultado.error) {
