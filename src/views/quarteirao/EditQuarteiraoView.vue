...
import { useAtribuirData } from '@/composables/atribuiData';
...
async function loadData() {
  ...
  try {
    const response = await quarteiraoService.getQuarteirao(id_quarteirao.value);
    useAtribuirData(toRefs(quarteirao), response.data);
  } catch (error) {
    message.value = error.data || 'Erro ao carregar dados';
...
function update() {
  v$.value.$touch()
  if (!v$.value.$invalid) {
    quarteiraoService.update(quarteirao).then(
      (response) => {
        showMessage.value = true;
...
onMounted(() => {
  ...
  id_quarteirao.value = route.params.id;
  loadData();
});
</script>
