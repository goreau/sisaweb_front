      ...
      <div class="column is-11">
        ...
        <div class="card" style="min-height: 60vh">
          <header class="card-header">
            <p class="card-header-title is-centered">Visitas a Imóveis Cadastrados</p>
...
async function onEditRow(item) {
  const ret = await vc_imovelService.getVcImovel(item.row.id)
  if (ret.error) {
    toast.error(ret.msg)
  } else {
    store.setVisita({ ...ret })
    router.push({ name: 'imCadastrado', query: { from: 'edit' } })
  }
}
...
async function onDeleteRow(item) {
  ...
  if (ok) {
    const resultado = await vc_imovelService.delete(item.row.id)
    if (resultado.error) {
