      ...
      <div class="column is-11">
        <div class="card" style="min-height: 60vh">
          <header class="card-header">
            <p class="card-header-title is-centered">Borrifação Residual Intradomiciliar - BRI</p>
...
async function onEditRow(item) {
  const ret = await vc_briService.getVcBri(item.row.id)
  if (ret.error) {
    toast.error(ret.msg)
  } else {
    store.setFolha({ ...ret })
    router.push({ name: 'vcBri', query: { from: 'edit' } })
  }
}
...
async function onDeleteRow(item) {
  ...
  if (ok) {
    const resultado = await vc_briService.delete(item.row.id)
    if (resultado.error) {
