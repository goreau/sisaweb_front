...
<script setup>
import mobAladoService from '@/services/mobile/mobAlado.service'
...
async function onEditRow(item) {
  router.push(`/mobAlado/${item.row.id}`)
}
...
async function onDeleteRow(item) {
  try {
    ...
    if (ok) {
      const resultado = await mobAladoService.delete(item.row.id)
      if (resultado.error) {
...
async function sincroniza() {
  try {
    ...
    if (tabelaRef.value) {
      ...
      const ret = await mobAladoService.sync(linhas)
      if (ret.error) {
        toast.error(ret.msg)
      } else {
