      ...
      <div class="column is-11">
        ...
        <div class="card" style="min-height: 60vh">
          <header class="card-header">
            <p class="card-header-title is-centered">Exclusão de Ovitrampas Cadastradas</p>
            ...
            <section v-if="hasRows">
              ...
              <div class="columns">
                <div class="column is-4 is-offset-1">
                  <div class="content">
                    <label class="label">Ovitrampa a excluir</label>
                    <div class="control">
                      <CmbGeneric
                        v-enter-to-next="'form-imovel'"
                ...
                <div class="column is-4 is-offset-2">
                  <div class="content">
                    <label class="label">Ovitrampa que receberá</label>
                    <div class="control">
                      <CmbGeneric
                        v-enter-to-next="'form-imovel'"
...
async function loadData() {
  try {
    ...
    const result = await ovitrampaService.getDuplica(JSON.stringify(filter))
    if (result.error) {
...
async function processa() {
  const ok = await confirmDialog.value.show({
    title: 'Transferência',
    message: `Toda referência ao cadastro ID ${regExclui.value} será transferida para o cadastro ID ${regRecebe.value}! Confirma?`,
    okButton: 'Confirmar',
  })
  if (ok) {
    const resultado = await ovitrampaService.troca({ sai: regExclui.value, fica: regRecebe.value })
    if (resultado.error) {
...
watch(
  () => filter.id_municipio,
  async (val) => {
    const filter = { id_municipio: filter.id_municipio }
