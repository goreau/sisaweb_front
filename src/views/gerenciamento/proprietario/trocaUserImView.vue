        ...
        <div class="card" style="min-height: 60vh">
          <header class="card-header">
            <p class="card-header-title is-centered">
              Trocar Proprietário de Registros - Imóveis Cadastrados
            </p>
          </header>
...
async function loadData() {
  ...
  const result = await vc_imovelService.getDuplica(JSON.stringify(filter))
  if (result.error) {
...
async function processa() {
  ...
  if (ok) {
    const resultado = await vc_imovelService.troca({ sai: regExclui.value, fica: regRecebe.value })
    if (resultado.error) {
